...
export default function Cards() {
 ...
 const handleInput = (e) => {
    const filter = cards.filter( 
        card => card.title.toLowerCase().includes(e.target.value))
        setFilterdCards(filter)
}
...
<input type="text" onInput={handleInput} style={{backgroundColor:'#F1ECE8'}}
 className="rounded-full h-7 w-40 p-2 text-sm text-amber-900 font-semibold focus:outline-amber-900
 md:w-44 md:h-8
 lg:w-52 lg:h-9 "/>

 <img src={search} alt="" className="absolute bottom-80  w-4 right-8
...
<div style={{backgroundImage : `url(${card.image})`}} className="
